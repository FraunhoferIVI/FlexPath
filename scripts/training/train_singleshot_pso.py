"""Train on a single-shot actor environment using the single-shot config.

This script is a Hydra entrypoint (config: ``configs/config_singleshot.yaml``)
and initializes a PSO training run using a pre-loaded single-shot actor model
wrapped with VSAC logic. It prepares datasets, optimizers and learning-rate
schedule, then runs training via ``PSO.learn()``.
...
- Wraps the actor with ``PSOActorWrapper`` and constructs a ``PSO`` instance.
...
This script uses Hydra for configuration. Typical invocation examples:
    ...
    python scripts/training/train_singleshot_pso.py experiment.output_dir=experiments/runs/myrun training.hyperparams.total_epochs=100
...
from src.models.policies.aggregated_pso import AggregatedPSO

from src.models.policies.pso_training.actor_wrapper import PSOActorWrapper
...
def main(cfg: DictConfig):
    ...
    # Create model
    logger.info("Initializing PSO model (AggregatedPSO)...")
