...
@torch.compile
def compute_gradual_std_mask_2D(
    logits: torch.Tensor,
    state: torch.Tensor,
    ...
    obstacle_map = torch.all(state == obstacle_color[None, :, None, None], axis=1)
    ...
    neighbor_region = compute_immediate_neighbor_region(path=base_path)
...
def sample_random(
    ...
    for i in range(B):
        candidates = region[i][0].nonzero()
        random_idxs = torch.randperm(len(candidates))

        for j in range(min(amount, len(random_idxs))):
            x, y = candidates[random_idxs[j]]
            out[i, 0, x, y] = value

    return out


def compute_immediate_neighbor_region(
    path: torch.Tensor
):
    ...
    Parameters:
    - path: Tensor with 1: path, 0: no path, shape=[B, 1, H, W]
    ...
    path_with_neighbor_region_inconsistent = F.conv2d(
...
def compute_path_boundary(
    ...
    Parameters:
    - path_neighbors: Tensor with 1: neighbor, 0: no neighbor, shape=[B, 1, H, W]
    - path: Tensor with 1: path, 0: no path, shape=[B, H, W]
    ...
    neighbors_with_neighbor_region_inconsistent = F.conv2d(
