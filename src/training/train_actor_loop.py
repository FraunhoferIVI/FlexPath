...
def train_actor(model, train_dataset, val_dataset, config, device="cuda", diagonal_movements_at_obstacle=False, compile_model: bool = True):
    """
    Main training loop for actor pretraining.

    Args:
        model: PathFindingNetworkUNET2 model
        train_dataset: ActorDataset for training
        val_dataset: ActorDataset for validation
        config: Hydra config object
        device: 'cuda' or 'cpu'
        diagonal_movements_at_obstacle: Whether the dataset labels were generated allowing diagonal movements at obstacles. Important for path optimality metrics.
