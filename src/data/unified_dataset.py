...
class VoxelGymDataset(Dataset):
    ...
    def _extract_start_end(self, image_tensor: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Returns:
            (start_indices, end_indices) each shape [2]
        """
...
class ActorDataset(VoxelGymDataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        """
        Returns next sample if current sample has no start/end position.

        Returns:
            Dict with keys:
                - images: torch.Tensor [3, H, W]
                - labels: torch.Tensor [H, W]
                - starts: torch.Tensor [2] - start coordinate
                - ends: torch.Tensor [2] - end coordinate
