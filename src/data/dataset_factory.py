...
class DatasetFactory:
    ...
    def create_actor_dataset(config: DictConfig, split: str) -> ActorDataset:
        """Create an ActorDataset from a Hydra config.

        Parameters
        ----------
        config : DictConfig
            Configuration with a ``data.dataset`` section. Supports data sources
            ``hf``, ``localnpz``, and ``localzarr``.
        split : str
            Dataset split name, typically ``"train"`` or ``"validation"``.

        Returns
        -------
        ActorDataset
            Instantiated dataset with the appropriate adapter and transforms.

        Raises
        ------
        ValueError
            If ``data.dataset.source`` is not one of the supported sources.
        """
        # Determine source
