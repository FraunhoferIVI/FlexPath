...
from src.differentiable_reward.rewards.util import is_connected__waypoint, is_collision, compute_path_cost_approximation, compute_soft_obstacle_distances, soft_optimal_connectivity
...
def reward_waypoint(
    ...
    _is_connected = (is_connected__waypoint(predicted_path, start=start, end=end, waypoint=waypoint) + is_connected__waypoint(predicted_path, start=end, end=start, waypoint=waypoint)) / 2
    ...
    obstacle_penalty = (obstacle_penalty_mean + obstacle_penalty_max) / 2  # [-1, 0]
...
def reward_waypoint_mindist(
    ...
    # pred_pixel_sum = torch.sum(predicted_path, dim=(1, 2, 3))  # [B, 1]
    # target_pixel_sum = torch.sum(target_path, dim=(1, 2, 3))  # [B, 1]

    # pixel_penalty = -torch.clamp(1 - (target_pixel_sum + eps) / (pred_pixel_sum + eps), min=0.0, max=1.0)
