"""
modified code from 
https://github.com/omron-sinicx/neural-astar/blob/minimal/src/neural_astar/planner/differentiable_astar.py

Extends implementation to handle non-uniform step costs (sqrt(2) for diagonal)
...
class DifferentiableAstar(nn.Module):
    def __init__(self, g_ratio: float = 0.5, Tmax: float = 1.0):
        ...
        # self.cost_filter = torch.tensor([[sqrt2, 1.0, sqrt2], [1.0, 0.0, 1.0], [sqrt2, 1.0, sqrt2]], requires_grad=False).cuda().view(1, 1, 3, 3)  # MOD
    ...
    def forward(
        ...
        assert torch.all(goal_maps.sum(dim=(-1,-2,-3)) == 1), "Goal map not one-hot!"
        assert torch.all(start_maps.sum(dim=(-1,-2,-3)) == 1), "Start map not one-hot!"
        ...
        for t in range(Tmax):
            ...
            g, open_maps, histories, parents, is_break = step(
                g,
                open_maps,
                goal_maps,
                cost_maps,
            ...
            # g2 = expand((g + cost_maps) * selected_node_maps, neighbor_filter) + compute_step_cost(selected_node_maps, self.cost_filter)
...
def step(
    ...
    # 1) neighbor is not in the close list (1 - histories) nor in the open list (1 - open_maps)
    # 2) neighbor is in the open list but g < g2
    g2 = expand((g + cost_maps) * selected_node_maps, neighbor_filter) + compute_step_cost(selected_node_maps, cost_filter)
    idx = (1 - open_maps) * (1 - histories) + open_maps * (g > g2)
    idx = idx * neighbor_nodes
    idx = idx.detach()
    g = g2 * idx + g * (1 - idx)
    ...
    is_break = torch.all(is_unsolved.flatten() == 0)

    return g, open_maps, histories, parents, is_break