"""Copied from https://github.com/sair-lab/iAstar/"""
...
class dastar(nn.Module):
    ...
    def get_heuristic(self,
        ...
        grid = torch.meshgrid(torch.arange(0, H, device=self.device), torch.arange(0, W, device=self.device))
    ...
    def forward(self,
        ...
        size_sqrt = math.sqrt(size[-2]*size[-1])

        for t in range(Tmax):

            open_maps, parents, g, h, cost_maps, goal_maps, obstacles_maps, histories, is_break = step(open_maps, parents, g, h, cost_maps, goal_maps, obstacles_maps, histories, ns, nc, self.num_samples, size_sqrt)

            if not is_break.item():
                continue
...
@torch.compile
def step(open_maps, parents, g, h, cost_maps, goal_maps, obstacles_maps, histories, ns, nc, num_samples, size_sqrt):
    # 71
    # f = self.g_ratio * g +(1 - self.g_ratio)*h
    # 72 73
    f = g + h + cost_maps
    f_exp = torch.exp(-1 * f/size_sqrt)
    f_exp = f_exp * open_maps
    node_selection = _st_softmax_noexp(f_exp)
    dist_to_goal = (node_selection*goal_maps).sum((1, 2), 
                                                    keepdim=True)
    ...
    # open neighboring nodes, add them to the openlist if they satisfy certain requirements
    neighbor_nodes = expand(node_selection, ns, num_samples)
    neighbor_nodes = neighbor_nodes * obstacles_maps
    ...
    # 2) neighbor is in the open list but g < g2
    g2 = (g*node_selection).sum((1, 2), keepdim=True)
    g2 = g2 + expand(node_selection*cost_maps, nc, num_samples)
    idx = (1 - open_maps) * (1 - histories) + open_maps * (g > g2)
    idx = idx * neighbor_nodes
    idx = idx.detach()
    g = g2 * idx + g * (1 - idx)
    ...
    snm = node_selection.reshape(num_samples, -1)
    new_parents = snm.max(-1, keepdim=True)[1]
    parents = new_parents * idx + parents * (1 - idx)
    is_break = torch.all(is_unsolved.flatten()<1e-6)

    return open_maps, parents, g, h, cost_maps, goal_maps, obstacles_maps, histories, is_break


def expand(x: torch.Tensor, neighbor_filter: torch.Tensor, num_samples) -> torch.Tensor:
    """
    Expand neighboring node
