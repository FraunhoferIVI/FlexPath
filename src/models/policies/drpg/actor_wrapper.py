...
Now, DRPGActorWrapper accepts any actor model, enabling architecture ablation studies.

Usage:
    from src.utils.checkpoint_loader import load_pretrained_actor
    from src.models.policies.vsac.actor_wrapper import DRPGActorWrapper
    ...
    wrapper = DRPGActorWrapper(actor_model, num_actions=1764)
...
from src.exploration.pixel_wise_changes_2D import compute_gradual_std_mask_2D
from src.exploration.pixel_wise_changes_3D import compute_gradual_std_mask_3D
...
class DRPGActorWrapper(nn.Module):
    """
    ...
    def __init__(self, actor_model: nn.Module, exploration_strategy: str = "v0", compile_model: bool = False):
        ...
        Example:
            ...
            >>> wrapper = DRPGActorWrapper(actor, num_actions=1764)
        """
        super().__init__()
    ...
    def forward_nonreasoning(self, state: torch.Tensor, std_decay: float = 1.0) -> tuple:
        ...
        elif self.exploration_strategy == "v3":
            current_add_pixel_ratio = self.add_pixel_ratio * (std_decay + 1) / 2 if self.decay else self.add_pixel_ratio
            if x.ndim == 3:
                # 2 spatial dims, 2D
                log_std = compute_gradual_std_mask_2D(
                    logits=x.unsqueeze(1),
                    state=raw_obs, 
                    add_pixel_ratio=current_add_pixel_ratio,
                    amount_of_pixels=self.amount_of_pixels_added_removed
                )  
            elif x.ndim == 4:
                # 3 spatial dims, 3D
                log_std = compute_gradual_std_mask_3D(
                    logits=x.unsqueeze(1),
                    state=raw_obs, 
                    add_pixel_ratio=current_add_pixel_ratio,
                    amount_of_pixels=self.amount_of_pixels_added_removed
                ...
                raise RuntimeError("Unsupported number of spatial dimensions, only 2D and 3D is currently supported.")
               
            # flatten to [B, H*W]
            log_std = log_std.view(log_std.size(0), -1)

        else:
            raise RuntimeError("No valid exploration strategy set in DRPGActorWrapper.")
