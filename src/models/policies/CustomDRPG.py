...
from src.differentiable_reward.differentiable_reward_function import DifferentiableRewardFunction

from src.differentiable_reward.reward_registry import REWARD_COMPONENTS
...
class DRPG(nn.Module):
    """
    DRPG: Differentiable Reward Policy Gradient trainer.
    ...
    reward_f : DifferentiableRewardFunction
    ...
    def __init__(
        ...
        super(DRPG, self).__init__()

        # ----------------------------------------------------------------------
        # Store policy and optimizer
        # ----------------------------------------------------------------------
        self.policy = SamplingActor(model=policy)  # Simple wrapper that only adds sampling capabilities, API: policy(input: torch.Tensor, deterministic: bool)
        ...
        self.reward_f = DifferentiableRewardFunction(
        ...
        for epoch in range(self.epochs):
            ...
            if (epoch - 1) % self.checkpoint_save_freq == 0:
                self.checkpoint_manager.save_checkpoint(
                    model=self.policy.actor_model.actor_model,
                    step=epoch,
                    metric_value=1, #mean_reward,
                )
